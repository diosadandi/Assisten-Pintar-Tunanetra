const char* ssid     = "Emilkharis";      //masukan ssid wifi   
const char* password = "kalimantan007";   //masukan password dari wifi
...
static const char PROGMEM INDEX_HTML[] = R"rawliteral(
  ...
  //<!-- memasukan atau import model dan Tensorflow.js -->
...
-->
  ...
  <script>
    ...
    //<!-- function yang akan menjalankan model -->
      ...
      Model.detect(canvas).then(Predictions => {    
        ...
        if (Predictions.length>0) {
          ...
          for (var i=0;i<Predictions.length;i++) {
            ...
            const object_name = Predictions[i].class;
            const predcition_score = Math.round(Predictions[i].score*100);

            result.innerHTML+= "[ "+i+" ] "+object_name+", "+predcition_score+"%, "+Math.round(x)+", "+Math.round(y)+", "+Math.round(width)+", "+Math.round(height)+"<br>";
           ...
           // result.innerHTML+= "[ "+i+" ] "+Predictions[i].class+", "+Math.round(Predictions[i].score*100)+"%, "+Math.round(x)+", "+Math.round(y)+", "+Math.round(width)+", "+Math.round(height)+"<br>";
            ...
            //Bikin Dictionary
             const dict_object = {object_name};
             const stringi = JSON.stringify(dict_object);

             console.log(stringi);
             
              // disini buat pengirim json
              console.log('send POST request')

              //masukan API yang diberikan oleh python
              
              try{
                fetch('http://192.168.1.7:5000/camera', {
                method: 'POST',
                headers: {
